...
X.volume.prototype.create_ = function(_scan_direction, _scan_direction_vec) {
  ...
  // scan direction int possible values
  // 1: AXIAL
  // 2: CORONAL
  // 3: SAGITTAL
  ...
  console.log('X: ' + this._slicesX._children.length);
  console.log('Y: ' + this._slicesY._children.length);
  console.log('Z: ' + this._slicesY._children.length);
  ...
  console.log('_spacing: ' + this._spacing);
  ...
  for (xyz = 0; xyz < 3; xyz++) {
    ...
    for (i = 0; i < this._dimensions[_xyz]; i++) {
      var _position = (-halfDimension * this._spacing[_xyz])
          + (i * this._spacing[_xyz]);
      var _center = new Array([ this._center[0] + _position, this._center[1],
          this._center[2] ], [ this._center[0], this._center[1] + _position,
