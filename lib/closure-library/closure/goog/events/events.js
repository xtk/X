...
/**
 * @fileoverview Event Manager. Provides an abstracted interface to the
 *               browsers' event systems. This uses an indirect lookup of
 *               listener functions to avoid circular references between DOM (in
 *               IE) or XPCOM (in Mozilla) objects which leak memory. This makes
 *               it easier to write OO Javascript/DOM code. It simulates capture &
 *               bubble in Internet Explorer. The listeners will also
 *               automagically have their event objects patched, so your
 *               handlers don't need to worry about the browser. Example usage:
 * 
 * <pre>
 * goog.events.listen(myNode, 'click', function(e) {
 * 
 *   alert('woo')
 * });
 ...
 * added once to an object and if it is added again the key for the listener is
 * returned.
 * 
 * @param {EventTarget|goog.events.EventTarget} src The node to listen to events
 *          on.
...
goog.events.getProxy = function() {
  ...
  var f = goog.events.BrowserFeature.HAS_W3C_EVENT_SUPPORT ? function(
      eventObject) {
 ...
 * 
 * @param {EventTarget|goog.events.EventTarget} src The target to stop listening
 *          to events on.
 * @param {string|Array.<string>} type The name of the event without the 'on'
 *          prefix.
 * @param {Function|Object} listener The listener function to remove.
 * @param {boolean=} opt_capt In DOM-compliant browsers, this determines whether
...
goog.events.removeAll = function(opt_obj, opt_type, opt_capt) {
  ...
  if (!noObj) {
    var srcUid = goog.getUid(/** @type {Object} */
    (opt_obj));
    if (goog.events.sources_[srcUid]) {
      var sourcesArray = goog.events.sources_[srcUid];
 ...
 * @param {?string} type Event type.
 * @param {boolean} capture Capture phase?.
 * @return {Array.<goog.events.Listener>?} Array of listener objects. Returns
 *         null if object has no listeners of that type.
 * @private
 */
...
/**
 * Gets the goog.events.Listener for the event or null if no such listener is in
 * use.
 * 
...
goog.events.getListener = function(src, type, listener, opt_capt, opt_handler) {
  ...
  if (listenerArray) {
    for ( var i = 0; i < listenerArray.length; i++) {
      ...
      if (!listenerArray[i].removed && listenerArray[i].listener == listener &&
          listenerArray[i].capture == capture &&
          listenerArray[i].handler == opt_handler) {
        // We already have this listener. Return its key.
...
goog.events.fireListeners = function(obj, type, capture, eventObject) {
  ...
  if (type in map) {
    map = map[type];
    if (capture in map) {
      return goog.events.fireListeners_(map[capture], obj, type, capture,
          eventObject);
    }
  }
  return true;
...
/**
 * Dispatches an event (or event like object) and calls all listeners listening
 * for events of this type. The type of the event is decided by the type
 * property on the event object. If any of the listeners returns false OR calls
 * preventDefault then this function will return false. If one of the capture
 * listeners calls stopPropagation, then the bubble listeners won't fire.
 * 
 * @param {goog.events.EventTarget} src The event target.
 * @param {string|Object|goog.events.Event} e Event object.
 * @return {boolean} If anyone called preventDefault on the event object (or if
 *         any of the handlers returns false) this will also return false. If
 *         there are no handlers, or if all handlers return true, this returns
 *         true.
 */
goog.events.dispatchEvent = function(src, e) {
  ...
  if (hasCapture) {
    ...
    for ( var i = ancestors.length - 1; !e.propagationStopped_ && i >= 0 &&
        targetsMap.remaining_; i--) {
      e.currentTarget = ancestors[i];
      rv &= goog.events.fireListeners_(targetsMap, ancestors[i], e.type, true,
          e) &&
  ...
  if (hasBubble) {
    ...
    if (hasCapture) { // We have the ancestors.
      ...
      for ( var current = src; !e.propagationStopped_ && current &&
          targetsMap.remaining_; current = current.getParentEventTarget()) {
        e.currentTarget = current;
        rv &= goog.events.fireListeners_(targetsMap, current, e.type, false, e) &&
            e.returnValue_ != false;
      }
    }
  }
...
goog.events.protectBrowserEventEntryPoint = function(errorHandler) {
  ...
  goog.events.handleBrowserEvent_ = errorHandler
      .protectEntryPoint(goog.events.handleBrowserEvent_);
};


/**
 * Handles an event and dispatches it to the correct listeners. This function is
 * a proxy for the real listener the user specified.
 * 
 * @param {string} key Unique key for the listener.
 * @param {Event=} opt_evt Optional event object that gets passed in via the
 *          native event handlers.
 * @return {boolean} Result of the event handler.
 * @this {goog.events.EventTarget|Object} The object or Element that fired the
 *       event.
 * @private
 */
  ...
  if (!goog.events.BrowserFeature.HAS_W3C_EVENT_SUPPORT) {
    ...
    try {
      if (hasCapture) {
        ...
        // Call capture listeners
        for ( var i = ancestors.length - 1; !evt.propagationStopped_ &&
            i >= 0 && targetsMap.remaining_; i--) {
        ...
        if (hasBubble) {
          ...
          for ( var i = 0; !evt.propagationStopped_ && i < ancestors.length &&
              targetsMap.remaining_; i++) {
            evt.currentTarget = ancestors[i];
            retval &= goog.events.fireListeners_(targetsMap, ancestors[i],
                type, false, evt);
          }
        }
