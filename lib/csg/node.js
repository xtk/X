...
csgNode.prototype = {
  ...
  clipPolygons: function(polygons) {
    ...
    for ( var i = 0; i < polygons.length; i++) {
      this.plane_.splitPolygon(polygons[i], front, back, front, back);
    }
    if (this.front_) {
  ...
  build: function(polygons) {
    ...
    if (!this.plane_) {
      this.plane_ = polygons[0].plane().clone();
    }
    var front = [], back = [];
    for ( var i = 0; i < polygons.length; i++) {
      this.plane_.splitPolygon(polygons[i], this.polygons_, this.polygons_, front,
          back);
    }
    if (front.length) {
      if (!this.front_) {
