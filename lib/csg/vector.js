...
csgVector = function(ix, iy, iz) {
  ...
  if (arguments.length == 3) {
    ...
    this.y_ = Number(iy);
    this.z_ = Number(iz);
  } else if ('x' in ix && 'y' in ix && 'z' in ix) {
    this.x_ = Number(ix.x());
    this.y_ = Number(ix.y());
    this.z_ = Number(ix.z());
  } else {
    this.x_ = Number(ix[0]);
    window.console.log("x: " + this.x_);
    this.y_ = Number(ix[1]);
    this.z_ = Number(ix[2]);
  }
};
...
csgVector.prototype = {
  ...
  cross: function(a) {
    ...
    return new csgVector(this.y_ * a.z() - this.z_ * a.y(), this.z_ * a.x() - this.x_ *
        a.z(), this.x_ * a.y() - this.y_ * a.x());
  },
