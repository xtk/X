...
X.parserVTK.prototype.parse = function(container, object, data, flag) {
  ...
  var header = this.parseChars(_data, 0, 50 ).split("\n");

  // VTK files may be ASCII or Binary...
  if ( header[2] == "BINARY") {
    this.parseBinaryVTK(container, object, data);
    // Guess as to how many points we may need;
    object._points = p = new X.triplets(this._unorderedPoints.count*50);
    object._normals = n = new X.triplets(this._unorderedNormals.count*50);
  } else {
 ...
 */
 X.parserVTK.prototype.parseBinaryVTK = function(container, object, data) {
