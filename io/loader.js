 ...
 *         mimetype, additional (optional) flags which are passed to the parser,
 *         the response type]
 * @throws {Error} An error, if the configured file format is not supported.
 */
X.loader.prototype.checkFileFormat = function(container, mimetype) {

  // get the associated file of the object
  var filepath = container._file._path;

  // grab the file extension
  var extension = container._file.getExtension();

  // If a mimetype or extesion is provided and not in the supported formats, throw.
  if (extension || mimetype)
  {
    if (!(extension in X.loader.mimetypeByExtension) && !(mimetype in X.loader.mimetypes))
    {
      throw new Error('The ' + (mimetype || extension) + ' file format is not supported.');
    }
  }

  // Extension first, then mimetype, default to dicom (as before)
  // Wold prefer mimetype precidence, but that would break existing behavior.
  mimetype = X.loader.mimetypeByExtension[extension] || mimetype || 'application/dicom';

  var parserInfo = X.loader.mimetypes[mimetype];

  return [container._file._path, mimetype,
          parserInfo.parser[0], parserInfo.parser[1], parserInfo.parser[2]];
};
...
X.loader.prototype.parse = function(request, container, object) {
  ...
  setTimeout(function() {
    ...
    // parser
    var _checkresult = this.checkFileFormat(container, request.getResponseHeader('Content-Type'));
...
X.loader.mimetypes = {
  'application/vnd.ms-pki.stl': {
    extensions: ['STL'],
    parser: [X.parserSTL, null]
  },
  'application/dicom': {
    extensions: ['DCM', 'DICOM'],
    parser: [X.parserDCM, null]
  },
  'application/vnd.proteus.magazine': {
    extensions: ['MGH'],
    parser: [X.parserMGZ, false]
  },
  'application/vnd.proteus.magazine.zip': {
    extensions: ['MGZ'],
    parser: [X.parserMGZ, true]
  },
  'application/x-colormap-lookup': {
    extensions: ['TXT', 'LUT'],
    parser: [X.parserLUT, null]
  },
  'application/x-freesurfer-curvature': {
    extensions: ['CRV'],
    parser: [X.parserCRV, null]
  },
  'application/x-freesurfer-label': {
    extensions: ['LABEL'],
    parser: [X.parserLBL, null]
  },
  'application/x-freesurfer-mesh': {
    extensions: ['FSM', 'INFLATED', 'SMOOTHWM', 'SPHERE', 'PIAL', 'ORIG'],
    parser: [X.parserFSM, null]
  },
  'application/x-nrrd': {
    extensions: ['NRRD'],
    parser: [X.parserNRRD, null]
  },
  'application/x-nifti': {
    extensions: ['NII'],
    parser: [X.parserNII, null]
  },
  'application/x-nifti-compressed': {
    extensions: ['GZ'],
    parser: [X.parserNII, null]
  },
  'application/x-obj': {
    extensions: ['OBJ'],
    parser: [X.parserOBJ, null]
  },
  'application/x-trackvis': {
    extensions: ['TRK'],
    parser: [X.parserTRK, null]
  },
  'application/x-vtk': {
    extensions: ['VTK'],
    parser: [X.parserVTK, null]
  },
  'image/png': {
    extensions: ['PNG'],
    parser: [X.parserIMAGE, 'png']
  },
  'image/jpeg': {
    extensions: ['JPG', 'JPEG', 'JPE'],
    parser: [X.parserIMAGE, 'jpeg']
  },
  'image/gif': {
    extensions: ['GIF'],
    parser: [X.parserIMAGE, 'gif']
  }
};

/**
 * Supported mimetypes be extension
 *
 * @enum {Array}
 */
X.loader.mimetypeByExtension = {};
// Build by extension for fallback lookup
var mimetype, attrs, extension;
for (mimetype in X.loader.mimetypes)
{
  attrs = X.loader.mimetypes[mimetype];
  while(attrs.extensions.length)
  {
    extension = attrs.extensions.pop();
    X.loader.mimetypeByExtension[extension] = mimetype;
  }
}